#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into Pi's skills directory."""
...
def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "pi")
...
def _validate_skill_name(name: str) -> None:
    ...
    if len(name) > 64:
        raise InstallError("Skill name must be 64 characters or fewer.")
    if not re.fullmatch(r"[a-z0-9-]+", name):
        raise InstallError(
            "Skill name must contain only lowercase letters, numbers, and hyphens."
        )
    if name.startswith("-") or name.endswith("-"):
        raise InstallError("Skill name must not start or end with a hyphen.")
    if "--" in name:
        raise InstallError("Skill name must not contain consecutive hyphens.")
...
def _default_dest() -> str:
    return _pi_skills_dir()


def _parse_args(argv: list[str]) -> Args:
    parser = argparse.ArgumentParser(description="Install a Pi skill from GitHub.")
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        for path in source.paths:
            ...
            if os.path.exists(dest_dir):
                raise InstallError(f"Destination already exists: {dest_dir}")
            install_targets.append((path, skill_name, dest_dir))
        tmp_dir = tempfile.mkdtemp(prefix="skill-install-", dir=_tmp_root())
        try:
            repo_root = _prepare_repo(source, args.method, tmp_dir)
            installed = []
            for path, skill_name, dest_dir in install_targets:
